...
from tools_my.transformer_dec import TransformerDecoder
...
class AttentionBlock(nn.Module):
    ...
    def _forward(self, x):
        ...
        x = x.reshape(b, c, -1)
        print("x reshape is", x.shape)
        qkv = self.qkv(self.norm(x))
        # kv = self.kv(self.norm(x))
        # q = self.q(self.norm(x))
        # print("kv shape is", kv.shape)
    ...
    #     return x
    # TransformerDecoder(dim=dim, depth=self.dec_depth,
    #                    heads=8, dim_head=self.decoder_dim_head, mlp_dim=mlp_dim, dropout=0,
    #                    softmax=decoder_softmax)

if __name__ == "__main__":
    dim = 32
    attblock = TransformerDecoder(dim=dim, depth=1,
                heads=8, dim_head=64, mlp_dim=2*dim, dropout=0.,
                softmax=True)
